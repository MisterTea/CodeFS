...
void inotify_monitor::scan(const string& path, const bool accept_non_dirs) {
  ...
  if (follow_symlinks && S_ISLNK(fd_stat.st_mode)) {
    string link_path;
    if (read_link_path(path, link_path)) scan(link_path, accept_non_dirs);
...
void inotify_monitor::preprocess_dir_event(struct inotify_event* event) {
  ...
  if (flags.size()) {
    impl->events.push_back(
        {impl->wd_to_path[event->wd], impl->curr_time, flags});
  }
...
void inotify_monitor::process_pending_events() {
  ...
  std::for_each(impl->paths_to_rescan.begin(), impl->paths_to_rescan.end(),
                [this](const string& p) { this->scan(p); });
...
void inotify_monitor::run() {
  ...
  for (;;) {
    ...
    int rv = select(impl->inotify_monitor_handle + 1, &set, nullptr, nullptr,
                    &timeout);
    ...
    ssize_t record_num =
        read(impl->inotify_monitor_handle, buffer, BUFFER_SIZE);
